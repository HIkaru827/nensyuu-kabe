          ...
          <Card className="border-slate-200 bg-slate-50">
            <CardContent className="pt-6 space-y-3">
              ...
              <ul className="space-y-2 text-sm text-slate-700">
                <li>・ 「178万円の壁」は、主に本人の所得税の話です</li>
                <li>・ 税法上の扶養判定の目安は、給与収入 136万円です</li>
                <li>・ 19歳以上23歳未満は、197万円以下で特定親族特別控除の対象になり得ます</li>
          ...
          <Card>
            <CardContent className="pt-6 space-y-8">
              <section className="space-y-3">
                <h2 className="text-2xl font-bold text-foreground">178万円の壁は何が変わったのか</h2>
                <p className="text-sm leading-relaxed text-muted-foreground">
                  令和8年分以後の所得税では、令和8・9年分の給与所得控除の最低保障が 74万円になり、
                  基礎控除も低所得帯では 104万円に引き上げられています。
                  この2つを合わせると、給与収入ベースで 178万円まで所得税が発生しない設計になります。
                </p>
                <p className="text-sm leading-relaxed text-muted-foreground">
                  ただし、これはあくまで <strong className="text-foreground">所得税</strong> の話です。
                  親の扶養判定や社会保険まで 178万円にそろって変わったわけではありません。
                  なお、令和8年11月までの源泉徴収事務は改正前の税額表で進み、年末調整で精算される点にも注意が必要です。
                </p>
              </section>

              <section className="space-y-3">
                <h2 className="text-2xl font-bold text-foreground">税法上の扶養は 136万円が目安</h2>
                <p className="text-sm leading-relaxed text-muted-foreground">
                  扶養控除の判定は、扶養される人の合計所得金額で見ます。令和8年分以後はこの基準が 62万円以下となるため、
                  給与収入だけなら目安は <strong className="text-foreground">136万円以下</strong> です。
                </p>
                <p className="text-sm leading-relaxed text-muted-foreground">
                  つまり、178万円まで所得税が発生しなくても、税法上の扶養判定は 136万円を境に別で見直す必要があります。
                </p>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-lg border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
                  <ol className="space-y-2">
                    <li>1. 所得税を見るなら 178万円ラインを確認する</li>
                    <li>2. 親の税法上の扶養を見るなら 136万円、19歳以上23歳未満なら 197万円まで確認する</li>
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-3">
                  <a href="https://www.nta.go.jp/users/gensen/2026kiso/index.htm" target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-border p-4 hover:border-primary">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-foreground">国税庁 令和8年度税制改正</p>
                        <p className="text-xs text-muted-foreground">基礎控除、給与所得控除、扶養親族等の所得要件改正</p>
                      </div>
                      <ExternalLink className="h-4 w-4 shrink-0 text-muted-foreground" />
                    </div>
                  </a>
                  <a href="https://www.nta.go.jp/publication/pamph/gensen/2026kaisei.pdf" target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-border p-4 hover:border-primary">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-foreground">国税庁 源泉所得税の改正のあらまし</p>
                        <p className="text-xs text-muted-foreground">178万円、136万円、197万円の前提になる改正表</p>
                      </div>
                      <ExternalLink className="h-4 w-4 shrink-0 text-muted-foreground" />
                    </div>
                  </a>
                  <a href="https://www.nta.go.jp/taxes/shiraberu/taxanswer/shotoku/1180.htm" target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-border p-4 hover:border-primary">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">扶養判定の基本要件</p>
                      </div>
                      <ExternalLink className="h-4 w-4 shrink-0 text-muted-foreground" />
                    </div>
                  </a>
                  <a href="https://www.nta.go.jp/taxes/shiraberu/taxanswer/shotoku/1177.htm" target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-border p-4 hover:border-primary">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">19歳以上23歳未満の段階控除</p>
                      </div>
                      <ExternalLink className="h-4 w-4 shrink-0 text-muted-foreground" />
                    </div>
                  </a>
