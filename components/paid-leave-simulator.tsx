...
import { CalendarCheck, CircleAlert, CircleCheck, Clock, ExternalLink, PiggyBank } from "lucide-react"
          ...
          <CardContent className="space-y-5 p-5">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="as-of-date" type="date" value={asOfDate} onChange={(event) => setAsOfDate(event.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>契約日数の入力方法</Label>
                <Select value={scheduleBasis} onValueChange={(value) => setScheduleBasis(value as PaidLeaveScheduleBasis)}>
                  <SelectTrigger className="w-full" data-testid="schedule-basis-trigger">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {scheduleBasis === "weekly" ? (
                <div className="space-y-2">
                  ...
                  <Select value={String(weeklyWorkDays)} onValueChange={(value) => setWeeklyWorkDays(Number(value))}>
                    <SelectTrigger className="w-full" data-testid="weekly-work-days-trigger">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {[1, 2, 3, 4, 5].map((days) => (
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  有給日に支払われる金額は、勤務先が就業規則などで定める方式で変わります。
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select value={payMethod} onValueChange={(value) => setPayMethod(value as PaidLeavePayMethod)}>
                    <SelectTrigger className="w-full" data-testid="pay-method-trigger">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
